...
public class RouletteWheel extends ParentSelection {
	...
	public void selectCompetitors(Genotype[] pop) {
		...
		if (length == 0)
			...
			if (Population.testParameters.parentSelectionMode == Test.FITNESS_PROPORTIONATE)
				fitnessProportionateInitialization(proportion, fitness, sumFitness);
			else if (Population.testParameters.parentSelectionMode == Test.SIGMA_SCALING)
				sigmaScalingInitialization(proportion, fitness, sumFitness);
			else if (Population.testParameters.parentSelectionMode == Test.RANK)
